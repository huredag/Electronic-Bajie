...
volatile static csi_ospi_event_t ospi_event;
static void ospi_event_cb(csi_ospi_t *ospi, csi_ospi_event_t event, void *arg)
{
    ospi_event = event;
}
...
static int init(void)
{
    ...
    csi_ospi_attach_callback(&ospi, ospi_event_cb, NULL);
    return 0;
}
...
static int send(const void *data, uint32_t size)
{
    ...
    ospi_event = -1;
    ret = csi_ospi_send_async(&ospi, data, size);
    ...
    while (ospi_event != OSPI_EVENT_SEND_COMPLETE) {
        if ((csi_tick_get_ms() - timestart) > EXAMPLE_FLASH_TX_RX_TIMEOUT) {
            return -1;
        }
    }
...
static int receive(void *data, uint32_t size)
{
    ...
    ospi_event = -1;
    ret = csi_ospi_receive_async(&ospi, data, size);
    ...
    while (ospi_event != OSPI_EVENT_RECEIVE_COMPLETE) {
        if ((csi_tick_get_ms() - timestart) > EXAMPLE_FLASH_TX_RX_TIMEOUT) {
            return -1;
        }
    }

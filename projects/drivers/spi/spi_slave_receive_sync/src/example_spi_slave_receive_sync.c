...
/******************************************************************************
 * @file     example_spi_slave_receive_sync.c
 * @brief
 * @version
 * @date     2020-02-12
 ******************************************************************************/
...
int example_spi_receive_sync(void)
{
    int      ret = 0;
    ...
    ret = csi_spi_receive(&spi_slave, slave_rx_data, TEST_STR_LENGTH, -1);
    if (ret < CSI_OK) {
        return -1;
    }

    if (memcmp(slave_rx_data, TEST_STR, TEST_STR_LENGTH) != 0) {
...
int main(void)
{
    ...
    printf("-------------------------------------\n");
    printf("example_spi_slave_receive_sync()\n");
    printf("-------------------------------------\n");
    ret = example_spi_receive_sync();

    if (ret < 0) {
        printf("run example_spi_receive_sync fail\n");
    }

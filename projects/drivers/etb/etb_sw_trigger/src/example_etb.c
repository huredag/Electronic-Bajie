...
static int32_t etb_ch0, etb_ch1;
static int32_t etb0_config(void)
{
    int32_t ret = CSI_OK;
    csi_etb_config_t etb_config;
    etb_ch0 = csi_etb_ch_alloc(ETB_CH_ONE_TRIGGER_ONE);
    ...
    etb_config.trig_mode = ETB_SOFTWARE_TRIG;
    csi_etb_ch_config((int32_t)etb_ch0, &etb_config);

    return ret;
}

static int32_t etb1_config(void)
{
    int32_t ret = CSI_OK;
    csi_etb_config_t etb_config;

    etb_ch1 = csi_etb_ch_alloc(ETB_CH_ONE_TRIGGER_ONE);
    ...
    etb_config.trig_mode = ETB_SOFTWARE_TRIG;
    etb_config.src_ip = (uint8_t)(timer_hs_num[TEST_TIM0]);
    etb_config.dst_ip = (uint8_t)(timer_stop_ht_num[TEST_TIM1]);
...
static void timer_event_cb_reload_fun(csi_timer_t *timer_handle, void *arg)
{
    ...
    printf("ETB trigger starting timer\n");
}
...
static int32_t test_user_defined_fun(csi_timer_t *timer_handle, csi_timer_t *timer1_handle)
{
    ...
    while(!csi_timer_get_remaining_value(timer1_handle));

    printf("ETB trigger starting timer\n");


    csi_etb_ch_start(etb_ch1);
    old_value  = csi_timer_get_remaining_value(timer1_handle);
    uint32_t new_value = old_value;
    do {
        new_value = csi_timer_get_remaining_value(timer1_handle);
    } while (new_value != old_value);

    printf("ETB trigger stopping timer\n");
...
static int32_t csi_timer_test_reload_fun(uint8_t timer1_num, uint8_t timer2_num)
{
    ...
    state = csi_timer_attach_callback(&g_timer1_handle, timer_event_cb_reload_fun, NULL);

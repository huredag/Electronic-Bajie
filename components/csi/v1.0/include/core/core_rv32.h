...
/**
  \brief   Set Interrupt Priority thresh
  \details Set the priority of an interrupt.
  \note    The priority cannot be set for every core interrupt.
  \param [in]      IRQn  Interrupt number.
  \param [in]  priority  Priority to set.
 */
__STATIC_INLINE void csi_vic_set_thresh(uint32_t priority)
{
    uint8_t nlbits = (CLIC->CLICINFO & CLIC_INFO_CLICINTCTLBITS_Msk) >> CLIC_INFO_CLICINTCTLBITS_Pos;
    CLIC->MINTTHRESH = ((priority << (8 - nlbits)) & 0xFFU) << 24U;
}
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_enable_profile (uint32_t cache_type, uint32_t op, uint32_t counter_type)
{
	if (!counter_type) {
    	if(!cache_type) {
			__set_MHPMEVENT3(0x1);
		} else {
			if (!op) {
				__set_MHPMEVENT14(0xc);
			} else {
				__set_MHPMEVENT16(0xe);
			}
		}
	} else {
    	if(!cache_type) {
			__set_MHPMEVENT4(0x2);
		} else {
			if (!op) {
				__set_MHPMEVENT15(0xd);
			} else {
				__set_MHPMEVENT17(0xf);
			}
		}
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_disable_profile (uint32_t cache_type, uint32_t op, uint32_t counter_type)
{
	if (!counter_type) {
    	if(!cache_type) {
			__set_MHPMEVENT3(0x0);
		} else {
			if (!op) {
				__set_MHPMEVENT14(0x0);
			} else {
				__set_MHPMEVENT16(0x0);
			}
		}
	} else {
    	if(!cache_type) {
			__set_MHPMEVENT4(0x0);
		} else {
			if (!op) {
				__set_MHPMEVENT15(0x0);
			} else {
				__set_MHPMEVENT17(0x0);
			}
		}
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_reset_profile (uint32_t cache_type, uint32_t op, uint32_t counter_type)
{
	if (!counter_type) {
    	if(!cache_type) {
			__set_MHPMCOUNTER3H(0x0);
			__set_MHPMCOUNTER3(0x0);
		} else {
			if (!op) {
				__set_MHPMCOUNTER14H(0x0);
				__set_MHPMCOUNTER14(0x0);
			} else {
				__set_MHPMCOUNTER16H(0x0);
				__set_MHPMCOUNTER16(0x0);
			}
		}
	} else {
    	if(!cache_type) {
			__set_MHPMCOUNTER4H(0x0);
			__set_MHPMCOUNTER4(0x0);
		} else {
			if (!op) {
				__set_MHPMCOUNTER15H(0x0);
				__set_MHPMCOUNTER15(0x0);
			} else {
				__set_MHPMCOUNTER17H(0x0);
				__set_MHPMCOUNTER17(0x0);
			}
		}
	}
}
...
/**
  ...
  */
__STATIC_INLINE uint64_t csi_cache_get_access_time (uint32_t cache_type, uint32_t op)
{
	uint64_t times = 0;
    if(!cache_type) {
		times = ((uint64_t)__get_MHPMCOUNTER3H() << 32) | __get_MHPMCOUNTER3();
	} else {
		if (!op) {
		times = ((uint64_t)__get_MHPMCOUNTER14H() << 32) | __get_MHPMCOUNTER14();
		} else {
		times =	((uint64_t)__get_MHPMCOUNTER16H() << 32) | __get_MHPMCOUNTER16();
		}
	}
    return times;
}
...
/**
  ...
  */
__STATIC_INLINE uint64_t csi_cache_get_miss_time (uint32_t cache_type, uint32_t op)
{
	uint64_t times = 0;
    if(!cache_type) {
		times = ((uint64_t)__get_MHPMCOUNTER4H() << 32) | __get_MHPMCOUNTER4();
	} else {
		if (!op) {
		times = ((uint64_t)__get_MHPMCOUNTER15H() << 32) | __get_MHPMCOUNTER15();
		} else {
		times =	((uint64_t)__get_MHPMCOUNTER17H() << 32) | __get_MHPMCOUNTER17();
		}
	}
    return times;
}

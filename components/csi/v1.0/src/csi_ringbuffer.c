...
/**
  ...
  */
static inline uint32_t csi_ringbuffer_size(csi_ringbuffer_t *fifo)
{
    return fifo->size;
}
...
{
    return csi_ringbuffer_size(fifo) - csi_ringbuffer_len(fifo);
}
...
{
    return (csi_ringbuffer_len(fifo) == 0U);
}
...
{
    return (csi_ringbuffer_avail(fifo) == 0U);
}
...
/**
  ...
  *         of bytes copied.
  */
uint32_t csi_ringbuffer_in(csi_ringbuffer_t *fifo, const void *datptr, uint32_t len)
{
    uint32_t writelen = 0U, tmplen = 0U;

    if (csi_ringbuffer_is_full(fifo)) {
        writelen = 0U;
    } else {
...
/**
  ...
  */
uint32_t csi_ringbuffer_out(csi_ringbuffer_t *fifo, void *outbuf, uint32_t len)
{
    uint32_t readlen = 0U, tmplen = 0U;

    if (csi_ringbuffer_is_empty(fifo)) {
        readlen = 0U;
    } else {

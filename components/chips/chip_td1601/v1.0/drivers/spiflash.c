    ...
    do {
        ...
        switch (line) {
            ...
            case SPIFLASH_DATA_4_LINES:
                ...
                } else if (param->qe_pos == 3U) {
                    *(uint16_t *) temp |= ((uint16_t)0x1U << param->qe_pos);
                    ret = csi_spiflash_write_reg(spiflash, 0xB1U, status, sr_count);
                } else {
                    *(uint16_t *) temp |= ((uint16_t)0x1U << param->qe_pos);
                    ret = csi_spiflash_write_reg(spiflash, 0x01U, status, sr_count);
                }

...
void wj_adc_edma_event_cb(csi_edma_ch_t *dma, csi_edma_event_t event, void *arg)
{
    csi_adc_t      *adc;
...
static csi_error_t wj_adc_start_edma(csi_adc_t *adc)
{
    csi_edma_ch_config_t config;
    wj_adc_regs_t       *adc_base;

    adc_base = (wj_adc_regs_t *)HANDLE_REG_BASE(adc);
    memset(&config, 0, sizeof(csi_edma_ch_config_t));

    /* configure dma channel */
    config.src_inc = EDMA_ADDR_CONSTANT;
    config.dst_inc = EDMA_ADDR_INC;
    config.src_tw  = EDMA_DATA_WIDTH_32_BITS;
    config.dst_tw  = EDMA_DATA_WIDTH_32_BITS;

    config.group_len = 4U;
    config.trans_dir = EDMA_PERH2MEM;
    config.handshake = 4U;
    csi_edma_ch_config(adc->edma, &config);

    wj_adc_config_os_channel_nums(adc_base, (uint32_t)adc->priv);

    /* set rx mode*/
    soc_dcache_clean_invalid_range((unsigned long)adc->data, adc->num * 4U);
    csi_edma_ch_start(adc->edma, (void *) & (adc_base->ADC_DR), adc->data, adc->num * 4U);
...
csi_error_t csi_adc_link_dma(csi_adc_t *adc, csi_adc_dma_t *adc_dma)
{
    CSI_PARAM_CHK(adc, CSI_ERROR);
    ...
    if (adc_dma != NULL) {
        if (adc_dma->dma) {
            adc_dma->dma->parent = adc;
            ret = csi_dma_ch_alloc(adc_dma->dma, -1, -1);

            if (ret == CSI_OK) {
                csi_dma_ch_attach_callback(adc_dma->dma, wj_adc_dma_event_cb, NULL);
                adc->dma   = adc_dma->dma;
                adc->start = wj_adc_start_dma;
                adc->stop  = wj_adc_stop_dma;
            } else {
                adc_dma->dma->parent = NULL;
            }
        } else if (adc_dma->edma) {
            adc_dma->edma->parent = adc;
            ret = csi_edma_ch_alloc(adc_dma->edma, -1, -1);

            if (ret == CSI_OK) {
                csi_edma_ch_attach_callback(adc_dma->edma, wj_adc_edma_event_cb, NULL);
                adc->edma   = adc_dma->edma;
                adc->start = wj_adc_start_edma;
                adc->stop  = wj_adc_stop_dma;
            } else {
                adc_dma->edma->parent = NULL;
            }
        }
    } else {
        if (adc->dma) {
        ...
        if (adc->edma) {
            csi_edma_ch_free(adc->edma);
            csi_edma_ch_detach_callback(adc->edma);
            adc->edma = NULL;
        }

...
void wj_dma_set_groupmode(wj_dma_ch_regs_t *addr, uint32_t src_inc, uint32_t dst_inc, uint32_t preemption, uint8_t group_len)
{
    ...
    if (group_len <= 16) {
        addr->CH_GLE &= ~(uint32_t)(WJ_DMA_CH_GLE_Msk);
        ...
        addr->CH_CTRLA &= ~(uint32_t)(WJ_DMA_CH_CTRLA_GROUP_LEN_Msk);
        addr->CH_CTRLA |= (uint32_t)((((uint32_t)group_len - 1U) & 0xFU) << WJ_DMA_CH_CTRLA_GROUP_LEN_Pos);
        addr->CH_GLE &= ~(uint32_t)(WJ_DMA_CH_GLE_Msk);
        addr->CH_GLE |= (uint32_t)((((uint32_t)group_len - 1U) & 0x30U) >> 4);
    }
}

...
ATTRIBUTE_DATA int32_t dw_uart_config_baudrate(dw_uart_regs_t *uart_base, uint32_t baud, uint32_t uart_freq)
{
    ...
    if (ret == 0) {
        ...
        if (dw_uart_get_ip_id(uart_base) == DW_UART_SUPPORT_RATE) {
            ...
            float divisor_all = (float)(uart_freq) / (float)(baud << 4U);
            divisor = (uint32_t)divisor_all;
            uint32_t divisor_f = (uint32_t)((float)(divisor_all - divisor) *  DLF_SIZE * 10U);
            if ((divisor_f % 10U) >= 5U) {
                divisor_f = (divisor_f / 10U) + 1U;
            } else {
                divisor_f = divisor_f / 10U;
            }
            dw_uart_config_uart_baud(uart_base, divisor_f);
        } else {
            if ((divisor % 10U) >= 5U) {

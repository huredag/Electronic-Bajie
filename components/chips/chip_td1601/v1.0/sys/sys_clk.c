...
const system_clk_config_t system_config[] = {
    ...
    {CPU_204MHZ,    {1U, EHS_CLK, CLK_DIV1,  PLL_FN_34,    0U}, PLL_CLK, ILS_CLK, 1U, CLK_DIV2, CLK_DIV2},
...
{
    uint32_t div1;
    div1 = wj_get_mclk_div(PMU_REG_BASE);
    return g_system_clock / div1;
}
...
uint32_t soc_get_apb_freq(uint32_t idx)
{
    ...
    uint32_t div1;
    div1 = wj_get_mclk_div(PMU_REG_BASE);
    return (uint32_t)(g_system_clock / div / div1);
}
